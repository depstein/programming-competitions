...
bool findPath(int len, bool visited[], int maxlen)
{
	...
	for(edge* p = gr[v].next; p != 0; p = p->next) {
		if (!visited[p->w]) {
			visited[p->w] = true;
			path[len+1] = p->w;
			if (findPath(len+1, visited, maxlen))
				return true;
		}
	}
	return false;
...
int main()
{
	...
	while (n != 0) {
		...
		while (maxflow(minlen) < n) {
			...
			for(int i=1; i<=n; i++) {
				if (gr[sink-i].next != 0 && gr[sink-i].next->w == sink) {
			...
			for(int j=0; j<n*n; j++) {
				index++;
				if (gr[index].next != 0 && gr[index].next->w == sink-n*n)
					gr[index].next = 0;
				for(int k=0; k<base[j].nadj; k++)
					if (index+base[j].adj[k] < sink)
						gr[index].next = new edge(index+base[j].adj[k], gr[index].next); 
			...
			for(edge *p = gr[sink].next; p != 0; p = p->next) {
				int w = p->w;
				p->w += n*n;
				int v = p->w;
				deleteEdge(v, sink);
